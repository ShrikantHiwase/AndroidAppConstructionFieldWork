...
2. **New Issue** → title, Add demo GPS, Add demo photo → Save (duration feeds Pilot **issue create median**). Toggle cloud icon offline, create another issue, go online (auto-sync).
3. **Today's DPR** → weather, manpower, ≥1 activity (optional evidence photo), blockers if any → Save draft → add voice note (demo stub, or live mic with `USE_NATIVE_SENSORS=true`) → **Submit** (duration feeds Pilot **DPR submit median**) → **Share PDF** (or text) to WhatsApp.
