...
50. ~~Hinglish ARB Documents + Drawing pins~~ — Documents browser/upload/viewer + Drawings / pin chrome via ARB.
...
55. ~~Local demo field queue seed~~ — synced sample issues/RFIs for Open queue (mirrors firebase seed titles; no outbox).
56. ~~Local demo DPR seed~~ — yesterday submitted DPR + blockers for Digests/Weekly (today left empty for nudge).
57. ~~Local demo drawing pin seed~~ — seeded rebar punch on GA Plan Level 02 (`pin_seed_rebar` → `issue_seed_rebar`).
...
67. ~~Professional polish pass~~ — global error handlers + portrait lock; dark theme + Noto UI fonts; RFI home entry; sign-out / DPR-submit confirms; retry + pull-to-refresh; evidence thumbnails; docs `PopScope` back; login validation; Android label/adaptive icon/branded splash/R8 + upload-key wiring; iOS photo-library strings.
